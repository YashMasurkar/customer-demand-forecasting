"""Visualization module for time-series demand forecasting, baselines, Holt-Winters, and SARIMA."""
...
def plot_sarima_comparison(
    ...
    ax1.set_title(f"Customer Demand Forecasting — SARIMA Model vs Actuals (Weekly Quantity)", fontsize=14, fontweight="bold", pad=12)
    ax1.set_ylabel("Weekly Demand (Quantity)", fontsize=12, fontweight="normal")
    ax1.legend(loc="upper left", frameon=True, framealpha=0.95, fontsize=10)
    ax1.grid(True, linestyle="--", alpha=0.5)
...
def plot_all_models_comparison(
    ...
    if "seasonal_naive_pred" in df_pred.columns:
        ax1.plot(df_pred["week_start"], df_pred["seasonal_naive_pred"], label="Seasonal Naive (MAE: 65.63)", color="#2ca02c", linestyle="-.", linewidth=1.8, alpha=0.8)

    ax1.set_title("Test Period Forecast Comparison: Actuals vs Holt-Winters vs SARIMA vs Baselines (2017)", fontsize=14, fontweight="bold", pad=12)
    ax1.set_ylabel("Weekly Demand (Quantity)", fontsize=12, fontweight="normal")
    ax1.legend(loc="upper left", frameon=True, framealpha=0.95, fontsize=10)
    ax1.grid(True, linestyle="--", alpha=0.5)
