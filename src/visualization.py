"""Visualization module for time-series demand forecasting, baselines, and Holt-Winters comparisons."""
...
def plot_holt_winters_comparison(
    ...
    """Generate and save publication-quality plot comparing Holt-Winters, baselines, and actuals.

    Args:
        complete_df: Full complete weekly dataframe (columns: week_start, quantity).
        forecast_df: Test forecast dataframe with actuals, HW preds, PI bounds, and baseline preds.
        split_date_str: String date marking the start of the test split.
    ...
    # Holt-Winters point forecast and 95% prediction interval
    ax1.plot(
        df_pred["week_start"],
        df_pred["hw_pred"],
        label="Holt-Winters Forecast (Add Trend, Add Season)",
        color="#9467bd",
        linewidth=2.4
    )
    ...
    # Baseline forecasts for direct comparison
    if "seasonal_naive_pred" in df_pred.columns:
        ax1.plot(
            df_pred["week_start"],
            df_pred["seasonal_naive_pred"],
            label="Seasonal Naive (Lag-52)",
            color="#2ca02c",
            linestyle="-.",
            linewidth=1.8,
            alpha=0.8
    ...
    plt.savefig(path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"Holt-Winters comparison plot successfully saved to: {path}")
    return path
